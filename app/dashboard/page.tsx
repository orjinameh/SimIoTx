...
export default function Dashboard() {
 ...
 // Layout - use mounted + isMobile to avoid hydration mismatch
 ...
 const r = await fetch(`/api/webhooks/threshold?deviceId=${id}`);
 const d = await r.json();
 ...
 e.stopPropagation();
 await fetch(`/api/simulator/${d.deviceId}`, {
 method:'POST',
 headers:{'Content-Type':'application/json'},
 body:JSON.stringify({ action:d.isRunning?'stop':'start', chaosMode:chaosMap[d.deviceId], csvRows:csvRows.length?csvRows:undefined })
 });
 await loadDevices();
 };
 ...
 const d = await r.json();
 if (d.success) { await loadDevices(); navigate('devices'); }
 else alert(d.error);
 };
 ...
 setView(v);
 setMenuOpen(false);
 if (v !== 'devices') setSelectedId(null);
 };
 ...
 <button onClick={startAll} style={BTN(true)}> ALL</button>
 <button onClick={stopAll} style={BTN()}> ALL</button>
 </div>
 )}
 </div>
 ...
 style={{...CARD,borderLeft:`3px solid ${device.isRunning?'#00FF88':'#334155'}`,cursor:'pointer',borderColor:selectedId===device.deviceId?'rgba(0,255,136,0.4)':'rgba(255,255,255,0.08)'}}>
 ...
 <div style={{fontSize:9,letterSpacing:1,fontWeight:800,color:device.isRunning?'#080C10':'#475569',background:device.isRunning?'#00FF88':'transparent',border:`1px solid ${device.isRunning?'#00FF88':'#334155'}`,padding:'3px 8px',flexShrink:0}}>
 {device.isRunning ? 'LIVE' : 'OFF'}
 </div>
 </div>

 <div style={{display:'flex',gap:8,flexWrap:'wrap' as const}}>
 <button onClick={(e) => toggle(device, e)} style={BTN(device.isRunning)}>
 {device.isRunning ? ' STOP' : ' START'}
 </button>
 <button onClick={(e) => { e.stopPropagation(); setChaosMap(m=>({...m,[device.deviceId]:!m[device.deviceId]})); }} style={{...BTN(chaosMap[device.deviceId]),padding:'9px 12px',fontSize:10}}>
 {chaosMap[device.deviceId] ? 'CHAOS ON' : 'CHAOS'}
 </button>
 <button onClick={(e) => { e.stopPropagation(); csvRef.current?.click(); }} style={{...BTN(csvRows.length>0),padding:'9px 12px',fontSize:10}}>
 CSV
 </button>
 <button onClick={(e) => generateShare(device.deviceId, e)} style={{...BTN(),padding:'9px 12px',fontSize:10}}>
 SHARE
 </button>
 </div>
 ...
 {logs.length === 0
 ? <div style={{color:'#334155',textAlign:'center' as const,padding:20}}>Start simulator to see logs</div>
 : logs.map((log, i) => (
 ...
 <span style={{color:log.status==='sent'?'#00FF88':'#FF4444',marginLeft:6}}>[{log.status.toUpperCase()}]</span>
 </div>
 ...
 <div style={{display:'flex',gap:8}}>
 {['mqtt','http'].map(p => <button key={p} onClick={() => setNd(d=>({...d,protocol:p}))} style={BTN(nd.protocol===p)}>{p.toUpperCase()}</button>)}
 </div>
 </div>
 ...
 <div style={{display:'flex',gap:8,flexWrap:'wrap' as const}}>
 {[1000,2000,5000,10000].map(ms => <button key={ms} onClick={() => setNd(d=>({...d,interval:ms}))} style={BTN(nd.interval===ms)}>{ms/1000}s</button>)}
 </div>
 </div>
 </div>
 ...
 {!selected
 ? <div style={{...CARD,textAlign:'center' as const,padding:32,color:'#475569',fontSize:13}}>Go to Devices and select a device first</div>
 : <div style={CARD}>
 ...
 {['POST','GET'].map(m => <button key={m} onClick={() => setNw(w=>({...w,method:m}))} style={BTN(nw.method===m)}>{m}</button>)}
 </div>
 ...
 <button onClick={() => setMenuOpen(m=>!m)} style={{background:'transparent',border:'1px solid rgba(255,255,255,0.1)',color:'#94A3B8',width:36,height:36,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
 {menuOpen
 ...
 {menuOpen && (
 <div style={{background:'#0D1117',borderBottom:'1px solid rgba(255,255,255,0.08)',position:'sticky',top:57,zIndex:99,padding:'8px 0'}}>
 {NAV_ITEMS.map(([id,,label]) => (
 ...
 {/* Desktop sidebar */}
 <div style={{width:220,borderRight:'1px solid rgba(255,255,255,0.06)',minHeight:'100vh',flexShrink:0}}>
 <Sidebar/>
 </div>
